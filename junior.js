...
const parentlogincodebutton = document.getElementById(
  "parentlogin-code-button"
);
...
function randomlogincode() {
  ...
  var downloadTimer = setInterval(function () {
    //start  the countdown for 5 sec using setInterval
    if (timeleft <= 0) {
  ...
  var downloadTimerparent = setInterval(function () {
    if (timeleftparent <= 0) {
      ...
      document.getElementById(
        "parentdisplaylogincode"
      ).innerHTML = parentrandomNumber;
      document.getElementById("parenttimer").innerHTML =
        "0.0" + timeleftparent + "s";
    }
    timeleftparent -= 1;
  }, 1000);
}
...
function parentloginconformation() {
  ...
  if (parentrandomNumber != parentlogincode.value || parentname.value == "") {
    ...
    // alert("Loggedin")
    // displayusername.innerHTML = "Hi" + " " + username.value + " ;)";
    window.location.replace("parents.html");
  }
}
